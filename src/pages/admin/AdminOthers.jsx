...
export function AdminGalleryList() {
  ...
  const handleDelete = async (id, title) => {
    if (!window.confirm(`Hapus album "${title}"? Semua foto akan dihapus.`)) return
    await api.delete(`/gallery/${id}`)
    toast.success('Album dihapus'); load()
  }

  const handleToggle = async (g) => {
    await api.put(`/gallery/${g._id}`, { isPublished: !g.isPublished })
    toast.success(g.isPublished ? 'Dijadikan draft' : 'Diterbitkan'); load()
  }

  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-2xl font-bold text-navy-700">Galeri Foto</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-20"><p className="text-gray-400 font-serif text-xl mb-4">Belum ada album</p><Link to="/admin/gallery/new" className="inline-flex items-center gap-2 bg-gold-500 text-white px-5 py-2.5 rounded-xl text-sm font-semibold"><FiPlus size={14} /> Buat Album</Link></div>
      ) : (
            ...
            <div key={g._id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all">
              <div className="h-40 bg-gray-100 relative">
                ...
                <div className="absolute top-2 right-2 flex gap-1.5">
                  <button onClick={() => handleToggle(g)} className={`text-xs font-semibold px-2 py-1 rounded-full ${g.isPublished ? 'bg-emerald-500 text-white' : 'bg-white/90 text-gray-600'}`}>{g.isPublished ? 'Terbit' : 'Draft'}</button>
                </div>
              </div>
              <div className="p-4">
                <h3 className="font-semibold text-navy-700 text-sm mb-3 truncate">{g.title_id}</h3>
                <div className="flex gap-2">
                  <Link to={`/admin/gallery/edit/${g._id}`} className="flex-1 text-center py-2 bg-navy-50 text-navy-700 hover:bg-navy-100 rounded-lg text-xs font-semibold transition-all">Edit & Upload</Link>
...
export function AdminGalleryForm() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      if (isEdit) { await api.put(`/gallery/${id}`, form); toast.success('Album diperbarui'); load() }
      else { const { data } = await api.post('/gallery', form); toast.success('Album dibuat'); navigate(`/admin/gallery/edit/${data.data._id}`) }
  ...
  return (
    <AdminLayout>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-serif text-2xl font-bold text-navy-700">{isEdit ? 'Edit Album' : 'Album Baru'}</h1>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4">
          <form onSubmit={handleCreate} className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
            <div><label className={lbl}>Judul *</label><input className={inp} required value={form.title_id} onChange={e => setForm({...form, title_id: e.target.value})} /></div>
            <div><label className={lbl}>Title (EN)</label><input className={inp} value={form.title_en} onChange={e => setForm({...form, title_en: e.target.value})} /></div>
            <div><label className={lbl}>Keterangan</label><textarea className={inp} rows={3} value={form.description_id} onChange={e => setForm({...form, description_id: e.target.value})} /></div>
            <div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-navy-700">Foto ({gallery?.photos?.length || 0})</h3>
                <label className={`flex items-center gap-2 cursor-pointer px-4 py-2 text-sm font-semibold rounded-xl transition-all ${uploading ? 'bg-gray-100 text-gray-400' : 'bg-navy-50 text-navy-700 hover:bg-navy-100'}`}>
                  ...
                  <FiUploadCloud size={15} /> {uploading ? 'Mengupload...' : 'Upload Foto'}
                </label>
              </div>
              {gallery?.photos?.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-400">
                  ...
                  <p className="text-sm">Upload foto ke album ini</p>
                </div>
              )}
            </div>
          </div>
...
export function AdminVersesList() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); setSaving(true)
    try { await api.post('/verses', form); toast.success('Ayat ditambahkan'); load(); setForm({ text_id: '', text_en: '', reference_id: '', reference_en: '', date: '', theme_id: '' }) }
    catch (err) { toast.error(err.response?.data?.message || 'Gagal') } finally { setSaving(false) }
  }
  const handleDelete = async (id) => {
    if (!window.confirm('Hapus ayat ini?')) return
    await api.delete(`/verses/${id}`); toast.success('Ayat dihapus'); load()
  }
  ...
  return (
    <AdminLayout>
      <h1 className="font-serif text-2xl font-bold text-navy-700 mb-6">Ayat Harian</h1>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <h2 className="font-semibold text-navy-700 mb-4">Tambah Ayat</h2>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div><label className={lbl}>Tanggal (MM-DD) *</label><input className={inp} placeholder="01-01" required pattern="\d{2}-\d{2}" value={form.date} onChange={e => setForm({...form, date: e.target.value})} /></div>
            <div><label className={lbl}>Teks Ayat (ID) *</label><textarea className={inp} rows={3} required value={form.text_id} onChange={e => setForm({...form, text_id: e.target.value})} /></div>
            <div><label className={lbl}>Verse (EN) *</label><textarea className={inp} rows={3} required value={form.text_en} onChange={e => setForm({...form, text_en: e.target.value})} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className={lbl}>Referensi (ID)</label><input className={inp} placeholder="Yohanes 3:16" value={form.reference_id} onChange={e => setForm({...form, reference_id: e.target.value})} required /></div>
              <div><label className={lbl}>Reference (EN)</label><input className={inp} placeholder="John 3:16" value={form.reference_en} onChange={e => setForm({...form, reference_en: e.target.value})} required /></div>
            </div>
            <div><label className={lbl}>Tema</label><input className={inp} value={form.theme_id} onChange={e => setForm({...form, theme_id: e.target.value})} /></div>
            <button type="submit" disabled={saving} className="w-full flex items-center justify-center gap-2 bg-gold-500 hover:bg-gold-600 disabled:opacity-60 text-white font-semibold py-2.5 rounded-xl text-sm">
              <FiSave size={15} /> {saving ? 'Menyimpan...' : 'Simpan Ayat'}
            </button>
          </form>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="p-4 border-b border-gray-100 font-semibold text-navy-700">Daftar Ayat ({verses.length}/366)</div>
...
export function AdminAboutPage() {
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      if (data.success) { setForm(p => ({ ...p, logoUrl: data.url })); toast.success('Logo diupload') }
    } catch { toast.error('Gagal upload') } finally { setUploading(false) }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try { await api.put('/about', payload); toast.success('Data berhasil disimpan') }
    catch (err) { toast.error(err.response?.data?.message || 'Gagal') } finally { setSaving(false) }
  }
  ...
  return (
    <AdminLayout>
      <h1 className="font-serif text-2xl font-bold text-navy-700 mb-6">Informasi Gereja</h1>
      <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
        <div className="bg-white rounded-2xl border border-gray-100 p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label className={lbl}>Nama Gereja (ID)</label><input className={inp} value={form.churchName_id} onChange={e => setForm({...form, churchName_id: e.target.value})} /></div>
          <div><label className={lbl}>Church Name (EN)</label><input className={inp} value={form.churchName_en} onChange={e => setForm({...form, churchName_en: e.target.value})} /></div>
          ...
          <div><label className={lbl}>Visi (ID)</label><textarea className={inp} rows={3} value={form.vision_id} onChange={e => setForm({...form, vision_id: e.target.value})} /></div>
          <div><label className={lbl}>Vision (EN)</label><textarea className={inp} rows={3} value={form.vision_en} onChange={e => setForm({...form, vision_en: e.target.value})} /></div>
          <div><label className={lbl}>Misi (ID) — satu per baris</label><textarea className={inp} rows={5} value={form.mission_id} onChange={e => setForm({...form, mission_id: e.target.value})} /></div>
          <div><label className={lbl}>Mission (EN) — one per line</label><textarea className={inp} rows={5} value={form.mission_en} onChange={e => setForm({...form, mission_en: e.target.value})} /></div>
          <div className="sm:col-span-2"><label className={lbl}>Sejarah (ID)</label><textarea className={inp} rows={6} value={form.history_id} onChange={e => setForm({...form, history_id: e.target.value})} /></div>
        </div>

        <div className="bg-white rounded-2xl border border-gray-100 p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2"><label className={lbl}>Alamat</label><textarea className={inp} rows={2} value={form.address} onChange={e => setForm({...form, address: e.target.value})} /></div>
          ...
          <div><label className={lbl}>Jadwal Ibadah (ID)</label><input className={inp} value={form.serviceSchedule_id} onChange={e => setForm({...form, serviceSchedule_id: e.target.value})} placeholder="Sabtu, 09.00 - 12.00 WIB" /></div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <label className={lbl}>Logo Gereja</label>
          ...
          <label className="flex items-center gap-2 border-2 border-dashed border-gray-200 hover:border-gold-400 rounded-xl p-3 cursor-pointer transition-colors w-fit">
            ...
            <span className="text-sm text-gray-500">{uploading ? 'Uploading...' : 'Upload Logo'}</span>
          </label>
        </div>
        ...
        <button type="submit" disabled={saving} className="flex items-center gap-2 bg-gold-500 hover:bg-gold-600 disabled:opacity-60 text-white font-semibold px-8 py-3 rounded-xl transition-all">
          <FiSave size={16} /> {saving ? 'Menyimpan...' : 'Simpan Semua'}
        </button>
      </form>
    </AdminLayout>
  )

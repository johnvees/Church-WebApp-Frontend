...
export default function AdminDashboard() {
  ...
  const greeting = () => {
    ...
    if (h < 12) return t('admin.greeting.morning')
    if (h < 17) return t('admin.greeting.afternoon')
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between p-5 border-b border-gray-100">
            <h2 className="font-serif font-semibold text-navy-700">{t('admin.recentArticles')}</h2>
            ...
            {recentArticles.length > 0 ? recentArticles.map((a, i) => (
              <Link key={a._id} to={`/admin/articles/edit/${a._id}`} className="flex items-center gap-3 p-4 hover:bg-gray-50 transition-colors">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 capitalize">{a.isPublished ? t('admin.published') : t('admin.draft')}</p>
                </div>
                <FiArrowRight className="text-gray-300 shrink-0 ml-auto" size={14} />
              </Link>
            )) : (
              <div className="p-8 text-center text-gray-400 text-sm">{t('admin.noArticles')}</div>
            )}
          </div>
        </motion.div>
        ...
        >
          <h2 className="font-serif font-semibold text-navy-700 mb-4">{t('admin.quickActions')}</h2>
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: t('admin.writeArticle'), icon: FiFeather, path: '/admin/articles/new', color: 'bg-blue-50 text-blue-600 hover:bg-blue-100' },
              { label: t('admin.uploadGallery'), icon: FiImage, path: '/admin/gallery/new', color: 'bg-orange-50 text-orange-600 hover:bg-orange-100' },
              { label: t('admin.addContent'), icon: FiBook, path: '/admin/bacaan/new', color: 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' },
              { label: t('admin.manageMembers'), icon: FiUsers, path: '/admin/members', color: 'bg-purple-50 text-purple-600 hover:bg-purple-100' },
            ].map((item) => (
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              <Link to="/admin/verses" className="flex items-center justify-between p-3 rounded-xl bg-gold-50 hover:bg-gold-100 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-gold-700">{t('admin.manageDailyVerse')}</span>
                </div>
